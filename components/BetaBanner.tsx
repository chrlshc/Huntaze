...
export default function BetaBanner() {
  ...
  useEffect(() => {
    try {
      const dismissed = localStorage.getItem('beta_banner_dismissed') === '1'
  ...
  return (
    <div className="w-full bg-purple-600 text-white text-sm py-2 px-3 flex items-center justify-center gap-3">
      ...
      <button
        className="ml-4 underline/50 hover:underline"
        onClick={() => { try { localStorage.setItem('beta_banner_dismissed', '1') } catch {}; setVisible(false) }}
