export class HorizontalScaler {
    ...
    async deregisterServiceInstance(serviceName, instanceId) {
        try {
            const instances = this.serviceInstances.get(serviceName) || [];
    ...
    // Load Balancing
    async getServiceInstance(serviceName, context) {
    ...
    selectInstance(instances, strategy, context) {
    ...
    weightedRoundRobinSelection(instances, weights) {
        if (!weights) {
            return this.roundRobinSelection(instances);
        }
    ...
    leastConnectionsSelection(instances) {
        return instances.reduce((least, current) => (current.activeConnections || 0) < (least.activeConnections || 0) ? current : least);
    }
    consistentHashingSelection(instances, context, hashKey) {
        if (!context || !hashKey) {
            return this.roundRobinSelection(instances);
        }
    ...
    async evaluateScaling(serviceName, config, metrics, instances) {
    ...
    shouldScaleUp(config, cpuUtilization, memoryUtilization, responseTime) {
    ...
    shouldScaleDown(config, cpuUtilization, memoryUtilization, responseTime) {
    ...
    async scaleUp(serviceName, config) {
        try {
    ...
    async scaleDown(serviceName, config) {
        try {
            ...
            const instances = this.serviceInstances.get(serviceName) || [];
            const instanceToRemove = instances.reduce((least, current) => (current.activeConnections || 0) < (least.activeConnections || 0) ? current : least);
    ...
    async terminateServiceInstance(instanceId) {
    ...
    async drainInstance(serviceName, instanceId) {
    ...
    // Health Checks
    async initializeInstanceHealthCheck(serviceName, instance) {
        const strategy = this.loadBalancers.get(serviceName);
        if (!strategy?.healthCheckEnabled)
            return;
    ...
    updateScalingMetrics(serviceName, action) {
    ...
    async startAutoScalingMonitoring() {
    ...
    getAllScalingMetrics() {
        return new Map(this.scalingMetrics);
    }
    async updateAutoScalingConfig(serviceName, config) {
