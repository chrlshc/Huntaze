...
export const CACHE_KEYS = {
    ...
    USER_PROFILE: (userId) => `smart_onboarding:user_profile:${userId}`,
    USER_JOURNEY: (userId) => `smart_onboarding:user_journey:${userId}`,
    USER_PERSONA: (userId) => `smart_onboarding:user_persona:${userId}`,
    ENGAGEMENT_SCORE: (userId) => `smart_onboarding:engagement_score:${userId}`,
    // Content cache keys
    LEARNING_PATH: (pathId) => `smart_onboarding:learning_path:${pathId}`,
    STEP_CONTENT: (stepId) => `smart_onboarding:step_content:${stepId}`,
    CONTENT_RECOMMENDATIONS: (userId, stepId) => `smart_onboarding:content_recommendations:${userId}:${stepId}`,
    // ML model cache keys
    MODEL_PREDICTIONS: (userId, modelType) => `smart_onboarding:predictions:${modelType}:${userId}`,
    SUCCESS_PREDICTION: (userId) => `smart_onboarding:success_prediction:${userId}`,
    // System cache keys
    SYSTEM_METRICS: 'smart_onboarding:system_metrics',
    ACTIVE_EXPERIMENTS: 'smart_onboarding:active_experiments',
    MODEL_PERFORMANCE: (modelId) => `smart_onboarding:model_performance:${modelId}`
};
