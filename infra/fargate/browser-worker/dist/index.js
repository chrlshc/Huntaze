...
async function reportState(userId, state, errorCode) {
    try {
        if (!APP_ORIGIN || !WORKER_TOKEN)
            return;
...
async function firstVisible(scope, candidates, timeout = 10000) {
...
async function fillOneOf(scope, candidates, value, perCandidateTimeout = 8000) {
    for (const c of candidates) {
        const loc = typeof c === 'string' ? scope.locator(c) : c;
        try {
...
async function scopeForLogin(page) {
...
async function ensureEmailMode(page, scope) {
...
const emitBreakerStateMetric = (client) => (state) => {
    try {
        const data = [
            { MetricName: 'CircuitStateChange', Value: 1, Unit: 'Count', Dimensions: [{ Name: 'Client', Value: client }, { Name: 'State', Value: state }] },
            ...(state === 'open' ? [{ MetricName: 'CircuitOpenedCount', Value: 1, Unit: 'Count', Dimensions: [{ Name: 'Client', Value: client }] }] : []),
            ...(state === 'half-open' ? [{ MetricName: 'CircuitHalfOpenCount', Value: 1, Unit: 'Count', Dimensions: [{ Name: 'Client', Value: client }] }] : []),
            ...(state === 'closed' ? [{ MetricName: 'CircuitClosedCount', Value: 1, Unit: 'Count', Dimensions: [{ Name: 'Client', Value: client }] }] : []),
        ];
        cw.send(new PutMetricDataCommand({ Namespace: 'Huntaze/OFWorker', MetricData: data }));
    }
    catch { }
};
...
async function uploadTraceArtifacts(jobId, page, videoPath) {
    if (!TRACE_S3_BUCKET)
        return null;
...
async function uploadVideoArtifact(jobId, videoPath) {
    if (!TRACE_S3_BUCKET)
        return null;
    const now = new Date().toISOString().replace(/[:.]/g, '-');
    const keyMp4 = `${TRACE_S3_PREFIX}${jobId}/${now}/page.mp4`;
...
async function setLinkState(userId, state, errorCode) {
    try {
        const exprVals = {
            ':s': { S: state },
...
async function metricWith(name, value, unit, dims) {
    try {
        await brCW.exec(() => cw.send(new PutMetricDataCommand({
            Namespace: 'Huntaze/OFWorker',
            MetricData: [{
...
async function getCreds(secretId) {
...
async function setRequiresAction(userId, flag) {
...
async function ensureConnected(page, context) {
    try {
        ...
        const ofCookies = cookies.filter((c) => (c.domain || '').includes('onlyfans.com'));
...
async function run() {
    ...
    try {
        if (ACTION === 'login') {
            ...
            if (!CREDS_SECRET_ID)
                throw new Error('OF_CREDS_SECRET_ID missing');
            ...
            try {
                console.info('[LOGIN] frames =', (page.frames() || []).map((f) => f.url()));
            }
            catch { }
            let scope = await scopeForLogin(page);
            try {
                console.info('[LOGIN] after scopeForLogin url =', page.url());
            }
            catch { }
            let usedForm = false;
            try {
            ...
            await page.waitForURL((url) => {
                try {
                    return !/\/(login|auth)/i.test(new URL(url).pathname);
                }
                catch {
                    return false;
                }
            ...
            await brDDB.exec(() => ddb.send(new PutItemCommand({
                ...
                Item: {
                    userId: { S: USER_ID },
                    cookiesCipherB64: { S: Buffer.from(enc.CiphertextBlob).toString('base64') },
        ...
        if (ACTION === 'inbox') {
            ...
            const items = await page.evaluate(() => {
                const list = Array.from(document.querySelectorAll('[data-e2e="message-item"]'));
                return list.slice(0, 50).map(el => ({
                    id: el.getAttribute('data-id') || '',
                    author: el.querySelector('[data-e2e="author"]')?.innerText || '',
                    text: el.querySelector('[data-e2e="text"]')?.innerText || '',
                    ts: Date.now()
                }));
            });
        ...
        if (ACTION === 'send') {
            if (!CONVERSATION_ID || !CONTENT_TEXT)
                throw new Error('Missing CONVERSATION_ID or CONTENT_TEXT');
            // Guard: ensure session is valid before deep-linking to /my/messages
            if (!(await ensureConnected(page, context))) {
                await setLinkState(USER_ID, 'LOGIN_REQUIRED');
                await reportState(USER_ID, 'LOGIN_REQUIRED');
        ...
        try {
            ...
            if (videoPath) {
                ...
                if (v?.videoUrl)
                    console.log(JSON.stringify({ videoS3: v.videoUrl }));
            }
        }
        catch { }
