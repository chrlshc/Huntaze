...
export class HuntazeOfStack extends Stack {
  constructor(scope: cdk.App, id: string, props?: StackProps) {
    ...
    const loginFailuresMetric = new cloudwatch.Metric({ namespace: ns, metricName: 'LoginFailures', statistic: 'sum', period: Duration.minutes(60) });
    const memUsageMetric = new cloudwatch.Metric({ namespace: ns, metricName: 'MemoryUsageMB', statistic: 'max', period: Duration.minutes(5) });
    ...
    dashboard.addWidgets(
      new cloudwatch.GraphWidget({
        ...
        width: 12,
        left: [
          new cloudwatch.Metric({ namespace: ns, metricName: 'LoginSuccessCount', statistic: 'sum', period: Duration.minutes(5) }),
          new cloudwatch.Metric({ namespace: ns, metricName: 'LoginFailureCount', statistic: 'sum', period: Duration.minutes(5) }),
        ],
      }),
      new cloudwatch.GraphWidget({
        title: 'Action Duration (ms)',
        width: 12,
        left: [new cloudwatch.Metric({ namespace: ns, metricName: 'ActionDurationMs', statistic: 'avg', period: Duration.minutes(5) })],
      }),
      new cloudwatch.GraphWidget({
        title: 'Session Duration (ms) & Memory (MB)',
        width: 24,
        left: [
          new cloudwatch.Metric({ namespace: ns, metricName: 'SessionDurationMs', statistic: 'avg', period: Duration.minutes(5) }),
          new cloudwatch.Metric({ namespace: ns, metricName: 'MemoryUsageMB', statistic: 'max', period: Duration.minutes(5) }),
        ],
      })
    );
