...
export default function DashboardHuntazeAIPage() {
  ...
  const sendMessage = async () => {
    ...
    setTimeout(() => {
      let responseText = "I'll analyze your data and share detailed insights.";
      const lower = newMessage.text.toLowerCase();
      if (lower.includes('vip')) {
        responseText = "From what I'm seeing, you have several inactive VIPs. I recommend a personalized re-engagement campaign. Want me to draft the messages?";
      } else if (lower.includes('revenue')) {
        responseText = "Your revenue is trending up. To gain +30%, I suggest: +15% on top-performing PPVs, two premium bundles, and post-purchase upsells.";
      } else if (lower.includes('content')) {
        responseText = "Suggested schedule: Monday/Wednesday/Friday (teasers), Tuesday/Thursday (PPV), and weekend (live or bundle).";
      }
  ...
  const examplePrompts = [
    "📊 Analyze inactive VIP fans",
    "💰 Grow my revenue by 30%",
      ...
      <div className={`flex h-[calc(100vh-4rem)] ${darkMode ? 'bg-gray-900' : 'bg-gray-50'} rounded-xl overflow-hidden`}>
        ...
        <div className={`${sidebarOpen ? 'w-64' : 'w-0'} ${darkMode ? 'bg-gray-950' : 'bg-gray-900'} transition-all duration-300 overflow-hidden flex flex-col`}>
          <div className="p-3 border-b border-gray-800">
            <button onClick={startNewChat} className="shopify-btn shopify-btn--secondary shopify-btn--sm shopify-btn--full dark">
              <Plus size={16} />
              New conversation
            </button>
          </div>
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            ...
            {conversations.filter(c => c.date === 'Today' || c.date === 'Now').map(conv => (
              <button key={conv.id} onClick={() => setActiveConversation(conv.id)} className={`shopify-btn shopify-btn--tertiary shopify-btn--sm shopify-btn--full text-left dark ${activeConversation === conv.id ? 'bg-gray-800' : ''}`}>
          ...
          <div className="p-3 border-t border-gray-800">
            <button className="shopify-btn shopify-btn--tertiary shopify-btn--sm shopify-btn--full text-left dark">Settings</button>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className={`flex items-center justify-between px-4 py-3 border-b ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className="flex items-center gap-3">
              <button onClick={() => setSidebarOpen(!sidebarOpen)} className="shopify-btn shopify-btn--tertiary shopify-btn--xs">
                <Menu size={20} />
              </button>
              <span className={`font-semibold ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Huntaze AI Assistant</span>
            </div>
            <button onClick={() => setDarkMode(!darkMode)} className="shopify-btn shopify-btn--tertiary shopify-btn--xs">
          ...
          <div className={`border-t ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className="max-w-3xl mx-auto p-4">
              ...
              <p className={`text-xs text-center mt-3 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Huntaze AI can make mistakes. Double-check important information.</p>
            </div>
          </div>
        </div>
      </div>
