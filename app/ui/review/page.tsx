...
function collectRoutes(): TargetRoute[] {
  ...
  for (const r of [...base, ...extra]) {
    if (!dedup.has(r.path)) dedup.set(r.path, r)
  }
...
function joinUrl(base: string, path: string) {
  try {
    ...
    // relative base (e.g., /preview) → just join
...
export default function UIReviewPage() {
  ...
  const [routes, setRoutes] = useState<TargetRoute[]>(defaults)
  const framesRef = useRef<Map<string, HTMLIFrameElement>>(new Map())
  const [a11y, setA11y] = useState<Record<string, A11ySummary>>({})

  const gridCols = useMemo(() => {
    return `grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${Math.max(2, Math.min(5, cols))} gap-4`
  }, [cols])
    ...
    const interval = setInterval(() => {
      for (const [key, el] of framesRef.current.entries()) {
        try {
          ...
          if (dark) html.classList.add('dark-mode', 'theme-dark')
          else html.classList.remove('dark-mode', 'theme-dark')

          // quick a11y checks
          const imgNoAlt = doc.querySelectorAll('img:not([alt])').length
          const aCandidates = Array.from(doc.querySelectorAll('a')) as HTMLAnchorElement[]
          const aNoName = aCandidates.filter((a) => {
            const t = (a.textContent || '').trim()
            const labelled = a.getAttribute('aria-label') || a.getAttribute('aria-labelledby')
            return !(t.length > 0 || (labelled && labelled.length > 0))
          }).length
          const btnCandidates = Array.from(doc.querySelectorAll('button')) as HTMLButtonElement[]
          const btnNoName = btnCandidates.filter((b) => {
          ...
          // likely blocked by X-Frame-Options on prod; ok in dev
        }
      }
    }, 800)
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <p className="text-sm text-gray-600 mb-4">Preview key pages across themes and sizes. Note: iframes may be blocked in prod by X-Frame-Options; use locally for full previews.</p>

        <div className="flex flex-wrap items-center gap-3 mb-4">
          <label className="text-sm">Base URL</label>
          <input
            className="border rounded px-2 py-1 text-sm min-w-[240px]"
            placeholder="(optional) e.g. https://prod.app"
            value={baseUrl}
          ...
          <button
            ...
            onClick={() => {
              framesRef.current.forEach((f) => f?.contentWindow?.location.reload())
            }}
          >
            Reload All
          </button>
        ...
        <div className={gridCols}>
          {routes.map((r) => {
            const url = joinUrl(baseUrl, r.path)
            const k = r.path
            const s = a11y[k]
            return (
              <div key={k} className="border rounded-lg overflow-hidden bg-white">
                ...
                <div className="bg-gray-100">
                  <iframe
                    ref={(el) => {
                      if (el) framesRef.current.set(k, el)
                      else framesRef.current.delete(k)
                    }}
                    title={r.name}
                    src={url}
                    style={{ width: mobile ? 375 : '100%', height: mobile ? 640 : 420, border: '0' }}
                  />
                </div>
              </div>
            )
